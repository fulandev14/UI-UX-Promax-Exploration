    ...
    () => {
      ...
      media.add(
        ...
        ({ conditions }) => {
          ...
          const select = gsap.utils.selector(experienceRef);
          const background = select("[data-experience-background]");
          const label = select("[data-experience-label]");
          const headingLines = select("[data-experience-heading-line]");
          const statement = select("[data-experience-statement]");
          const copy = select("[data-experience-copy]");
          const quote = select("[data-experience-quote]");
          const steps = select("[data-experience-step]");
          ...
          if (!reduceMotion) {
            ...
            entryTimeline.addLabel("arrival");

            if (label.length) {
              entryTimeline.from(
                label,
                { autoAlpha: 0, y: travel * 0.6, duration: 0.6 },
                "arrival",
              );
            }
            ...
            if (statement.length) {
              entryTimeline.from(
                statement,
                { autoAlpha: 0, y: travel, duration: 0.65 },
                "arrival+=0.28",
              );
            }
            ...
            if (copy.length) {
              entryTimeline.from(
                copy,
                { autoAlpha: 0, y: travel * 0.75, duration: 0.6 },
                "arrival+=0.42",
              );
            }
            ...
            if (desktop && !coarsePointer && background.length) {
    ...
    >
      ...
      <div className="relative z-10 flex min-h-[56rem] flex-col px-5 py-12 sm:min-h-[62rem] sm:px-8 md:px-10 md:py-16 lg:min-h-dvh lg:px-[clamp(3rem,4.6vw,5rem)] lg:py-[clamp(3rem,5vh,4.5rem)]">
        <p
          ...
          className="font-mono text-xs font-medium uppercase leading-none tracking-[0.13em] text-[var(--text-inverse)] md:text-sm"
        >
        ...
        <div className="mt-9 max-w-[38rem] md:mt-12 lg:mt-[clamp(2.2rem,4vh,3.75rem)] lg:max-w-[45rem]">
          ...
          >
              <span
                data-experience-heading-line
                className="block w-max origin-left scale-x-[0.72] whitespace-nowrap sm:scale-x-[0.7] md:scale-x-[0.68] lg:scale-x-[0.67]"
              >
              A Room
            </span>
              <span
                data-experience-heading-line
                className="block w-max origin-left scale-x-[0.62] whitespace-nowrap sm:scale-x-[0.6] md:scale-x-[0.58] lg:scale-x-[0.55]"
              >
              That Listens.
            </span>
          </h2>
          ...
          <p
            data-experience-statement
            className="mt-6 max-w-[11ch] font-serif text-[3.1rem] italic leading-[0.92] tracking-normal text-[var(--text-inverse)] sm:max-w-[12ch] sm:text-[4rem] md:text-[4.9rem] lg:mt-7 lg:text-[4.45rem] xl:text-[5.35rem]"
          >
          ...
          <p
            ...
            className="mt-8 max-w-[35ch] font-mono text-xs leading-[1.75] text-[rgb(251_248_241_/_0.92)] md:text-sm lg:mt-9 lg:max-w-[34ch]"
          >
        ...
        <figure
          ...
          className="mt-12 max-w-[18rem] self-end text-right lg:absolute lg:right-[clamp(3.5rem,5.8vw,6.5rem)] lg:top-[58%] lg:mt-0 lg:max-w-[14rem] lg:-translate-y-1/2 xl:max-w-[15.5rem]"
        >
        ...
        <ol className="mt-16 grid gap-7 sm:grid-cols-2 lg:mt-auto lg:grid-cols-4 lg:gap-10 xl:gap-14">
          {experienceSteps.map((step) => (
            <li data-experience-step key={step.number} className="min-w-0">
